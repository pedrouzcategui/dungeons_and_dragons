...
class Character
{
    ...
    function __construct($id = 1, $name, $class, $chapter = 1, $dialogue_node = 1)
    {
    ...
    {
        $result = DB::query("SELECT * FROM characters WHERE id = 1");
    ...
    {
        $result = DB::query("SELECT * FROM characters ORDER BY id DESC");
        $characters = [];
        foreach ($result as $character) {
            array_push($characters, new self($character['id'], $character['name'], $character['class_id']));
        }
        return $characters;
    }
}
